...
- 修复 macOS 环境下因看门狗重连重构导致会话间歇性删除失败的问题。
- 修复 Joycode 供应商网络环境下，部分模型服务提供商模型列表加载异常。

## 1.0.2 - 2026-07-13

- 修复最新版 Codex 加载 `ngs-analysis` 插件时 `defaultPrompt` 长度超长导致的崩溃。
- 关闭 Joycode 供应商 Claude 3.7 系列模型的深度思考（Thinking）选项。
